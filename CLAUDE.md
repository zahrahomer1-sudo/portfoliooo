...
**No real media or case-study content exists yet.** Everything in
`public/media/` is generated by `scripts/generate-placeholders.mjs` — abstract
duotone fields, not photographs. Every project and album entry in
`src/content/site.ts` is a labelled placeholder. Do not invent clients,
testimonials, metrics, prices or project outcomes to fill them.
